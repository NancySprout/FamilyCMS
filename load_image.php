...
if (isset($_POST['submit'])) {
	...
	if($imageName) {			
		...
		if(isset($imagePropertyArray)) {
			...
			$query.=" user_id, width, height, description, type, image_name, safe_file_name ";
			$query.=") VALUES (";
			$query.=" {$userId},{$width}, {$height}, '{$description}', {$imageType}, '{$imageName}', '{$safeFileName}' ";
			$query.=")";
			$result=mysqli_query($connection,$query);
			$imageId=mysqli_insert_id($connection);
			
			check_result($result,"load_image.php:Could not load ".$imageName." into db: ".mysqli_error($connection));
								
			//Create destination path +filename for file system storage	
			$destinationFile=FS_PATH.$loggedInUser["user_dir"]."/".$safeFileName;				
